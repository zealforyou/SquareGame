...
public class BluetoothChatHelper {
            ...
            switch (msg.what) {
                case ChatConstant.MESSAGE_STATE_CHANGE:
                    if (mChatCallback != null) {
                        mChatCallback.connectStateChange((BleState) msg.obj);
                    }
                    break;
    ...
    public BluetoothChatHelper(IChatCallback<byte[]> chatCallback) {
        ...
        mBleState = BleState.STATE_NONE;
        this.mChatCallback = chatCallback;
    }

    public void setmChatCallback(IChatCallback<byte[]> mChatCallback) {
    ...
    public synchronized BluetoothChatHelper setState(BleState bleState) {
        mBleState = bleState;
        mHandler.obtainMessage(ChatConstant.MESSAGE_STATE_CHANGE, -1, -1, bleState).sendToTarget();
        return this;
    }
    ...
    public synchronized void start(boolean secure) {
        ...
        setState(BleState.STATE_LISTEN);
    ...
    public synchronized void connect(BluetoothDevice device, boolean secure) {
        ...
        if (mBleState == BleState.STATE_CONNECTING) {
            if (mConnectThread != null) {
                mConnectThread.cancel();
                mConnectThread = null;
            }
        ...
        setState(BleState.STATE_CONNECTING);
    }
    ...
    public synchronized void connected(BluetoothSocket socket,
        ...
        setState(BleState.STATE_CONNECTED);
    }
    ...
    public synchronized void stop() {
        ...
        setState(BleState.STATE_NONE);
    }

    public void write(byte[] out) {
        ConnectedThread r;
        synchronized (this) {
            if (mBleState != BleState.STATE_CONNECTED)
                return;
            r = mConnectedThread;
        }
        r.write(out);
