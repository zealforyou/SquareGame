...
import com.zz.squarebrick.particle.Utils;
import com.zz.squarebrick.particle.particle.ExplodeParticle;
    ...
    public Particle[][] generateParticles(List<Square.Cell> cells, Rect bound, int mRow, int cellWidth) {
        ...
        for (int row = 0; row < partH_Count; row++) { //行
            for (int column = 0; column < partW_Count; column++) { //列
                ...
                particles[row][column] = generateParticle(getColor(cells, cellCol, mRow), random);
            }
        }
        return particles;
    }
